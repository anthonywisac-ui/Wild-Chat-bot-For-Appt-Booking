...
#
# Instagram Messaging API (Direct). Uses the same graph.facebook.com Send API
# shape as Messenger, just scoped to the connected Instagram account's token.
# whatsapp_handlers.py routes here whenever the recipient id is prefixed "ig:".
#
# Instagram Direct's template support is more limited than Messenger's, so
# list-type interactives use the same numbered-text-menu fallback as wwebjs.
...
class InstagramProvider:
    ...
    async def send_file(self, to: str, file_path: str, caption: str = "") -> bool:
        ...
        NOTE: Instagram Direct's file-attachment support is more limited than
        Messenger's; if this fails in practice, send_image as a fallback may
        be needed, or simply send the confirmation details as text instead."""
    ...
    async def _send_attachment(self, to: str, file_path: str, attach_type: str, content_type: str, caption: str = "") -> bool:
        ...
        try:
            ...
            with open(file_path, "rb") as f:
                ...
                async with session.post(upload_url, data=form, params=params) as resp:
                    if resp.status >= 400:
                        ...
                        logger.error(f"[InstagramProvider] attachment upload failed {resp.status}: {text}")
                        return False
            ...
            logger.error(f"[InstagramProvider] send_image exception: {exc}")
            return False

    async def dispatch_payload(self, payload: dict) -> bool:
        """Accepts the same Meta-shaped interactive payload built in
        whatsapp_handlers.py and translates it for Instagram Direct."""
        to = payload.get("to", "")
        if not to:
            return False
        ...
        if menu_map:
            store_menu_map("instagram", to, menu_map)
    ...
    async def _post(self, url: str, payload: dict) -> bool:
        try:
            ...
            async with session.post(url, json=payload, headers=headers) as resp:
                if resp.status >= 400:
                    text = await resp.text()
                    logger.error(f"[InstagramProvider] send failed {resp.status}: {text}")
                    return False
                return True
        except Exception as exc:
            logger.error(f"[InstagramProvider] send exception: {exc}")
            return False
